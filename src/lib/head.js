...
/**
 * The social card for a path. Every page shared one image before this, so a
 * link to /work/aegis or /proof previewed as the home page — the wrong
 * headline on the one thing a reader sees before deciding to click.
 * Generated by scripts/gen-og.py; the home card doubles as the fallback.
 */
function ogImageFor(path) {
  if (path === '/') return `${SITE}/og/index.png`
  return `${SITE}/og/${path.replace(/^\//, '').replace(/\//g, '-')}.png`
}

export function metaFor(route) {
  const meta = resolveMeta(route)
  return { ...meta, ogImage: ogImageFor(meta.path) }
}
